...
exports.approveTransfer = exports.getTransfer = exports.getTransfers = exports.createTransfer = void 0;
...
const createTransfer = async (req, res) => {
    ...
    try {
        ...
        const [emp] = await conn.execute(`SELECT e.*, t.project_id
       FROM employees e
       JOIN teams t ON e.current_team_id = t.id
       WHERE e.id = ? AND e.current_team_id = ? AND t.project_id IN (${userProjects.map(() => '?').join(',')})`, [employee_id, from_team_id, ...userProjects]);
        if (!emp[0]) {
            await conn.rollback();
        ...
        const [destTeam] = await conn.execute('SELECT project_id FROM teams WHERE id = ? AND project_id IN (' + userProjects.map(() => '?').join(',') + ')', [to_team_id, ...userProjects]);
        if (!destTeam[0]) {
            await conn.rollback();
        ...
        const [uniqueSkills] = await conn.execute(`SELECT s.name
       ...
       AND (
       ...
       ) = 0`, [employee_id, from_team_id, employee_id]);
        const hasSkillGap = uniqueSkills.length > 0;
        const gapNote = hasSkillGap
            ? `Transfer leaves critical gap: ${uniqueSkills.map(s => s.name).join(', ')}`
            : null;
        ...
        if (role === 'vp')
            status = 'approved';
        const [result] = await conn.execute(`INSERT INTO transfers
         (employee_id, from_team_id, to_team_id, reason, effective_date, status, requested_by, skill_gap_flag, skill_gap_note)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [employee_id, from_team_id, to_team_id, reason, effective_date, status, requested_by, hasSkillGap ? 1 : 0, gapNote]);
        const transferId = result.insertId;
        // Create first approval record
        await conn.execute(`INSERT INTO transfer_approvals (transfer_id, approver_id, action)
       VALUES (?, ?, 'pending')`, [transferId, requested_by]);
        // Audit log
        await conn.execute(`INSERT INTO audit_logs (user_id, action, entity_type, entity_id, details)
       VALUES (?, 'transfer.created', 'transfer', ?, ?)`, [requested_by, transferId, JSON.stringify({ employee_id, from_team_id, to_team_id, status })]);
        await conn.commit();
        res.status(201).json({
...
const getTransfers = async (req, res) => {
  ...
  `;
    const params = [...userProjects, ...userProjects];
    if (status) {
        query += ' AND tr.status = ?';
        params.push(status);
    }
...
const getTransfer = async (req, res) => {
    ...
    try {
       ...
       WHERE tr.id = ?`, [id]);
        ...
        const [approvals] = await db_1.default.execute(`SELECT ta.*, u.name AS approver_name, r.name AS approver_role
       FROM transfer_approvals ta
       JOIN users u ON ta.approver_id = u.id
       JOIN roles r ON u.role_id = r.id
       WHERE ta.transfer_id = ?
       ORDER BY ta.created_at`, [id]);
...
const approveTransfer = async (req, res) => {
    ...
    try {
        await conn.beginTransaction();
        const [rows] = await conn.execute('SELECT * FROM transfers WHERE id = ?', [id]);
        ...
        if (newStatus === 'approved') {
            await conn.execute('UPDATE employees SET current_team_id = ? WHERE id = ?', [transfer.to_team_id, transfer.employee_id]);
            // Mark as executed
            newStatus = 'executed';
        }
        await conn.execute('UPDATE transfers SET status = ? WHERE id = ?', [newStatus, id]);
        await conn.execute(`INSERT INTO transfer_approvals (transfer_id, approver_id, action, note, actioned_at)
       VALUES (?, ?, ?, ?, NOW())`, [id, approver_id, action, note || null]);
        await conn.execute(`INSERT INTO audit_logs (user_id, action, entity_type, entity_id, details)
       VALUES (?, ?, 'transfer', ?, ?)`, [approver_id, `transfer.${action}`, id, JSON.stringify({ note, new_status: newStatus })]);
        await conn.commit();
