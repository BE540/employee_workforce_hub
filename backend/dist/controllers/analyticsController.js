...
exports.getSkillGaps = exports.getResourcePlanner = exports.getDashboard = void 0;
const db_1 = __importDefault(require("../config/db"));
// GET /api/analytics/dashboard
const getDashboard = async (req, res) => {
    const userProjects = req.userProjects || [];
    try {
        const [[headcount]] = await db_1.default.execute(`SELECT COUNT(*) AS total FROM employees e
       JOIN teams t ON e.current_team_id = t.id
       WHERE t.project_id IN (${userProjects.map(() => '?').join(',')})`, userProjects);
        const [[utilization]] = await db_1.default.execute(`SELECT ROUND(AVG(e.utilization_pct)) AS avg FROM employees e
       JOIN teams t ON e.current_team_id = t.id
       WHERE t.project_id IN (${userProjects.map(() => '?').join(',')})`, userProjects);
        const [[pending_transfers]] = await db_1.default.execute(`SELECT COUNT(*) AS total FROM transfers tr
       ...
       AND (ft.project_id IN (${userProjects.map(() => '?').join(',')})
            OR tt.project_id IN (${userProjects.map(() => '?').join(',')}))`, [...userProjects, ...userProjects]);
        const [[bench]] = await db_1.default.execute(`SELECT COUNT(*) AS total FROM employees e
       JOIN teams t ON e.current_team_id = t.id
       WHERE e.availability = 'immediate' AND e.utilization_pct < 50
       AND t.project_id IN (${userProjects.map(() => '?').join(',')})`, userProjects);
        // Team utilization breakdown - only for user's projects
        const [teams] = await db_1.default.execute(`SELECT t.name, ROUND(AVG(e.utilization_pct)) AS avg_util, COUNT(e.id) AS member_count
       FROM teams t LEFT JOIN employees e ON e.current_team_id = t.id
       WHERE t.project_id IN (${userProjects.map(() => '?').join(',')})
       GROUP BY t.id`, userProjects);
        // Skill concentration - only for user's projects
        const [concentration] = await db_1.default.execute(`SELECT s.name AS skill, COUNT(DISTINCT es.employee_id) AS holder_count
       FROM skills s
...
const getResourcePlanner = async (req, res) => {
    const userProjects = req.userProjects || [];
    try {
        // Get projects with team and employee counts
        const [projects] = await db_1.default.execute(`SELECT
       ...
       GROUP BY p.id`, userProjects);
        ...
        const [teams] = await db_1.default.execute(`SELECT
       ...
       GROUP BY t.id`, userProjects);
        ...
        const [employees] = await db_1.default.execute(`SELECT
       ...
       WHERE t.project_id IN (${userProjects.map(() => '?').join(',')})
       GROUP BY e.id
       ORDER BY e.utilization_pct DESC`, userProjects);
...
exports.getResourcePlanner = getResourcePlanner;
